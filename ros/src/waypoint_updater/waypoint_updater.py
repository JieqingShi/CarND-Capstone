...
class WaypointUpdater(object):
    ...
    def get_closest_waypoint_idx(self):
        x = self.pose.pose.position.x  # look in message definitions to check attributes
        y = self.pose.pose.position.y
        closest_idx = self.waypoint_tree.query([x, y], 1)[1]  # get closest index from KDtree

        # check if closest idx corresponds to the waypoint ahead or behind the vehicle
        closest_coord = self.waypoints_2d[closest_idx]
        prev_coord = self.waypoints_2d[closest_idx - 1]

        # use dot product for vectors constructed by the coordinates to check
